(function () {
    ...
    ClientFactory.$inject = ['APIFactory', '$timeout', '$q', 'ngNotify', '$http'];
    function ClientFactory(APIFactory, $timeout, $q, ngNotify, $http) {
        ...
        function SaveClient() {
            ...
            angular.forEach(ClientPhones, function (phone) {
                service.Client.ClientPhones.push({
                    'ClientRecord': null,
        ...
        function SendDelivery(clientRecord) {
            ...
            APIFactory.Post(service.apiUrl + '/SendDelivery', clientRecord).then(function (response) {
                if (response.data.code !== 'InvalidDropoffAddress') {
                    ngNotify.set('Your request has been received. ' +
                        'It will be picked up from ' +
                        response.data.delivery.pickupLocation.address, {
                            type: 'success',
                            duration: 6000,
                            position: 'top',
                            sticky: true,
